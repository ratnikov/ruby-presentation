...
\begin{frame}
  ...
  \begin{block}{What we really want is:}
	...
	\begin{itemize}
	  ...
	  \item<5-> builds description out of the name and author.
	\end{itemize}
      \item<6-> Include that functionality in the \texttt{Book} and \texttt{Game} classes.
    \end{itemize}
  \end{block}
  \begin{visibleenv}<7>
...
\begin{frame}
  ...
  \begin{block}{Game plan:}<2->
    \begin{itemize}
      \item<2-> Create \texttt{Catalogable} module that gives \texttt{name} functionality.
...
\begin{frame}[fragile]
  ...
  \begin{semiverbatim}
    ...
    \uncover<1->{    items.\alert<2>{map} \{ |b| \alert<3>{b.description} \}.\alert<4>{join ", "}}
    \uncover<1->{  end}
    \uncover<1->{end}
  \end{semiverbatim}
  \begin{visibleenv}<2->
  \begin{block}{Only things this \texttt{Library} implementation cares about:}
    \begin{itemize}
      ...
      \item<5-> Whatever \texttt{b.description} returns must be concatenatable by \texttt{join}.
    \end{itemize}
  \end{block}
  \end{visibleenv}
\end{frame}
