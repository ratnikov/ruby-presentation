...
\begin{frame}[t,fragile]
  ...
  \begin{example}
    \begin{semiverbatim}
      ...
      \uncover<1->{  def title; @title end}
      \uncover<1->{  def title=(new\_title)}
...
\begin{frame}[t,fragile]
  ...
  \begin{example}[Inheriting classes]
    \begin{semiverbatim}
      \uncover<1->{class \alert<2>{PhotoAlbum < Book}}
      \uncover<1->{  \alert<3>{attr\_accessor :pictures}}
      \uncover<1->{end}
    \end{semiverbatim}
  \end{example}

  \begin{itemize}
    \item<2> \texttt{PhotoAlbum} now inherits all instance methods from \texttt{Book} class.
    \item<3> And has an additional pictures accessor.
  \end{itemize}
\end{frame}
