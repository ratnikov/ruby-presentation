...
\begin{frame}
  ...
  \begin{block}{Why?}
      \begin{itemize}
      \visible<3->{\item Code dublication means you wrote it at least twice.}
      \visible<4->{\item Code dublication reduces clarity.}
      \visible<5->{\item Code dublication is much harder to keep in sync.}
    \end{itemize}
  \end{block}
  \end{visibleenv}
\end{frame}
...
\begin{frame}[t,fragile]
  ...
  \alert<2,3>{def} author\_js\alert<2,3>{\_tag} \marked<3>{author}
    \alert<2,3>{script\_tag} author\alert<2,3>{\_js(}\marked<3>{author}\alert<2,3>{)}
  end
  \alert<2,3>{def} book\_js\alert<2,3>{\_tag} \marked{book}; \alert<2,3>{script\_tag} book\alert<2,3>{\_js(} \marked<3>{book}\alert<2,3>{)} end
  \end{semiverbatim}
  \begin{itemize}
